...
namespace MinesweeperClassLibrary.BusinessLogicLayer
{
    public class BoardService
    {
        ...
        public void SetupBombs()
        {
            ...
            // use difficulty to decide how many bombs to place
            if (_board.Difficulty == 1)
            {
                bombCount = _board.Size;
            }
            else if (_board.Difficulty == 2)
            {
                bombCount = _board.Size + 3;
            }
            else
            {
                bombCount = _board.Size + 6;
            }
            ...
            while (bombsPlaced < bombCount)
            {
                ...
                // make sure we do not put a bomb twice in the same spot
                ...
                for (int col = 0; col < _board.Size; col++)
                {
                    ...
                    for (int i = -1; i <= 1; i++)
                    {
                        for (int j = -1; j <= 1; j++)
                        {
                            ...
                            {
                                if (_board.Cells[neighborRow, neighborCol].IsBomb == true)
                                {
                                    bombCount++;
                                }
                            }
        ...
        public bool VisitCell(int row, int col)
        {
            ...
            // if it is a bomb, reveal it and end the game
            if (cell.IsBomb)
            {
                cell.IsVisited = true;
                return true;
        ...
        public string DetermineGameState()
        {
            ...
            for (int row = 0; row < _board.Size; row++)
            {
                for (int col = 0; col < _board.Size; col++)
                {
                    ...
                    // safe cells still hidden means the game is not over yet
                    if (cell.IsBomb == false && cell.IsVisited == false)
                    {
                        allSafeCellsHandled = false;
                    }

                    // bombs must be flagged for a win with this version
