...
namespace MinesweeperGUI
{
    public partial class GameForm : Form
    {
        ...
        private void ShowGameResult()
        {
            ...
            {
                FrmPlayerName nameForm = new FrmPlayerName();
                ...
                if (nameForm.ShowDialog() == DialogResult.OK)
                {
                    ...
                    GameStat stat = new GameStat(
                        ...
                        playerName,
                        _score,
                        _board.EndTime - _board.StartTime
                    );

                    // next step will be Form4
                    FrmScoreBoard scoreForm = new FrmScoreBoard(stat);
                    scoreForm.Show();
                }
        ...
        {
            // convert the current board into json text
            string json = JsonConvert.SerializeObject(_board);

            // save the json text into a file
            File.WriteAllText("savedgame.json", json);

            // let the player know the save worked
            MessageBox.Show("Game saved successfully.");
        }

        private void btnLoadGame_Click_1(object sender, EventArgs e)
        {
            // make sure the save file exists first
            if (File.Exists("savedgame.json") == false)
            {
                MessageBox.Show("No saved game was found.");
                return;
            }

            // read the saved json text from the file
            string json = File.ReadAllText("savedgame.json");

            // convert the json text back into a board object
            BoardModel loadedBoard = JsonConvert.DeserializeObject<BoardModel>(json);

            // make sure the board loaded correctly
            if (loadedBoard == null)
            {
                MessageBox.Show("Saved game could not be loaded.");
                return;
            }

            // replace the current board with the loaded board
            _board = loadedBoard;

            // reconnect the service layer to the loaded board
            _service = new BoardService(_board);

            // rebuild and redraw the board
            BuildButtons();
            UpdateBoard();
            UpdateLabels();

            // let the player know the load worked
